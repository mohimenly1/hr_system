...
const filterData = () => {
    ...
    router.get(route('hr.employees.attendance.show', props.employee.id), params, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const exportReport = () => {
    let url = route('hr.employees.attendance.export', props.employee.id);
...
const exportAbsentDays = () => {
    let url = route('hr.employees.attendance.export-absent', props.employee.id);
    url += `?filter_type=${filterType.value}`;
    if (filterType.value === 'month') {
        url += `&month=${selectedMonth.value}`;
    } else {
...
<template>
    <Head :title="`سجل حضور - ${employee.user.name}`" />
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                <div class="flex items-center justify-between mb-6 pb-4 border-b">
                    <div>
                        ...
                        <p class="text-gray-500">{{ employee.job_title }} - {{ employee.department?.name }}</p>
                    </div>
                    <Link :href="route('hr.employees.index')" class="text-indigo-600 hover:text-indigo-800 font-medium">
