...
watch([search, departmentFilter], throttle(() => {
    router.get(route('hr.employees.index'), pickBy({
        search: search.value,
...
const syncEmployeeAttendance = (employeeId) => {
    ...
    router.post(route('hr.employees.attendance.sync', employeeId), {}, {
        preserveScroll: true,
        onFinish: () => { syncingEmployeeId.value = null; }
    });
};
// --- Helper Functions ---
...
<template>
    ...
    <HrLayout>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <div>
                        <select v-model="departmentFilter" class="border border-gray-300 rounded-md text-gray-800 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="">
                                {{ authUser.roles.includes('department-manager') ? 'الأقسام التي تديرها' : 'كل الأقسام' }}
                            </option>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr v-for="employee in employees.data" :key="employee.id" class="border-b hover:bg-gray-50">
                            <td class="py-3 px-4">
                                ...
                                <div v-if="employee.managed_departments.length > 0" class="text-xs text-indigo-600 font-semibold mt-1">
                                    <i class="fas fa-crown mr-1"></i> مدير: {{ employee.managed_departments.map(d => d.name).join(', ') }}
                                </div>
                            ...
                            <td class="py-3 px-4">
                               <div class="flex flex-wrap gap-1">
                                    <span v-for="role in employee.user.roles" :key="role.id" class="bg-gray-200 text-gray-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                                        {{ role.name }}
                                    </span>
                               </div>
                            </td>
                            ...
                            <td class="py-3 px-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-4 rtl:space-x-reverse">
                                    <button @click="openUserModal(employee.user)" class="text-gray-500 hover:text-cyan-600" title="عرض بيانات المستخدم">
                                        <i class="fas fa-user-check text-lg"></i>
                                    </button>
                                    <div class="flex items-center justify-center space-x-4"  v-if="authUser.roles.includes('admin') || authUser.roles.includes('hr-manager') || authUser.roles.includes('department-manager')" >
                                        <button
                                            @click="openFingerprintModal(employee)" 
                                            class="text-gray-500 hover:text-purple-600" title="تعديل رقم البصمة">
                                        <i class="fas fa-fingerprint text-lg"></i>
                                    </button>
                                    <Link v-if="authUser.roles.includes('admin') || authUser.roles.includes('hr-manager')" :href="route('hr.employees.attendance.show', employee.id)" class="text-gray-500 hover:text-green-600" title="عرض سجل الحضور">
            ...
            <!-- Pagination -->
            <div v-if="employees.data.length > 0" class="mt-6 flex justify-between items-center">
                <div class="text-sm text-gray-700">
                    عرض {{ employees.from }} إلى {{ employees.to }} من {{ employees.total }} نتيجة
                </div>
                <div class="flex">
                    <Link
        ...
        <div v-if="showFingerprintModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex justify-center items-center p-4">
            <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                ...
                <p v-if="selectedEmployee" class="mt-2 text-gray-600">للموظف: <span class="font-bold">{{ selectedEmployee.user.full_name }}</span></p>
