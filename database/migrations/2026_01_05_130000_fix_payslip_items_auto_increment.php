    ...
    public function up(): void
    {
        try {
                ...
                if ($col->COLUMN_NAME !== 'id') {
                    try {
                        ...
                        if (!empty($columnType)) {
                            ...
                            DB::statement("ALTER TABLE payslip_items MODIFY {$col->COLUMN_NAME} {$typeWithoutAuto}");
                        }
                    } catch (\Exception $e) {
            ...
            if (($hasPrimaryKey[0]->count ?? 0) == 0) {
                DB::statement('ALTER TABLE payslip_items ADD PRIMARY KEY (id)');
            }

            // Step 4: Get current max ID to set AUTO_INCREMENT properly
            $maxIdResult = DB::select("SELECT COALESCE(MAX(id), 0) as max_id FROM payslip_items");
            ...
            DB::statement('ALTER TABLE payslip_items MODIFY id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT');

            // Step 6: Set AUTO_INCREMENT starting point
            if ($nextId > 1) {
                DB::statement("ALTER TABLE payslip_items AUTO_INCREMENT = {$nextId}");
            }
        } catch (\Exception $e) {
            Log::warning('Could not set AUTO_INCREMENT for payslip_items.id: ' . $e->getMessage());
            ...
            try {
                ...
                Log::error('Failed to fix AUTO_INCREMENT for payslip_items.id: ' . $e2->getMessage());
            }
        }
    }
